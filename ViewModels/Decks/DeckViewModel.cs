...
public class DeckViewModel(Deck deck) : ViewModelBase, IDeckViewModel
{
    ...
    /// <inheritdoc />
    public string Name { get; } = deck.Name ?? string.Empty;

    /// <inheritdoc />
    public string Description { get; } = deck.Description ?? string.Empty;

    /// <inheritdoc />
    public byte[]? ImageThumb { get; } = deck.Image?.ThumbBytes ?? null;
}
